...
export function AdminProductsList({
  ...
  if (query.status === "pending") {
    // Real (translated) header over skeleton rows - the header text is already
    // available client-side, so there's no reason to grey it out.
    return (
      <div role="table" className="rounded-lg border flex-1 min-h-0 overflow-auto [scrollbar-gutter:stable]">
        ...
        {Array.from({ length: 8 }).map((_, i) => (
          <ProductTableSkeletonRow key={i} showActions showCreatedBy />
        ))}
      </div>
    );
  }
        ...
        {virtualizer.getVirtualItems().map((vRow) => {
          if (isSentinelIndex(vRow.index)) {
            return (
              ...
              >
                <ProductTableSkeletonRow showActions showCreatedBy />
              </div>
            );
          }
