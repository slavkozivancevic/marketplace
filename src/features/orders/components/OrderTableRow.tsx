...
import { ORDER_COLS } from "./OrderTableSkeleton";
...
export function OrderTableRow({ order }: { order: UserOrderListItem }) {
  ...
  const itemSummary = order.items
    .map((item) => {
      ...
      const variantLabel = getVariantLabel(item.variant, order.locale) ?? item.variant?.sku ?? null;
  ...
  return (
    ...
    <Link
      href={{ pathname: "/dashboard/orders/[id]", params: { id: order.id } }}
      className="contents"
    >
      <div
        role="row"
