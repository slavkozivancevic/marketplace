...
function buildCategoryPaths(
  ...
  function localized(c: CategoryRow): string {
    return getCategoryName({ name: c.name, translations: c.translations }, locale);
  }

  function pathName(c: CategoryRow): string {
    const parts: string[] = [localized(c)];
    let cur = c.parentId ? byId.get(c.parentId) : undefined;
    while (cur) {
      parts.unshift(localized(cur));
  ...
  return flat
    .map((c) => ({ id: c.id, name: localized(c), pathName: pathName(c) }))
...
export default async function BulkProductsPage() {
  ...
  const localizedBrands = brands
    .map((b) => ({
      id: b.id,
      name: getBrandName({ name: b.name, translations: b.translations }, locale),
    }))
